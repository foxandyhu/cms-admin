...
export class ModelService implements IBaseService {
   ...
   * @param downItemId
   */
  sortModel(upItemId: string, downItemId: string): Promise<boolean> {
   ...
   * 获得模型对应的模版集合信息
   */
  getModelTemplates(modelId: string): Promise<any> {
    return this.httpUtil.get(AppApi.CONTENT.model_template.replace('{:modelId}', modelId)).then(response => {
      return Promise.resolve(response);
    });
  }
}
