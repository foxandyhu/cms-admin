...
export class CommentConfigComponent extends BaseComponent implements OnInit {
  ...
  commentConfig: any = {openComment: '', needLoginComment: '', maxCommentLimit: ''}; //  评论配置
  ...
  getCommentConfig() {
    this.commentService.getCommentConfig().then(result => {
      if (result) {
        ...
        this.commentConfig.openComment = this.commentConfig.openComment + '';
        this.commentConfig.needLoginComment = this.commentConfig.needLoginComment + '';
      }
    });
  }
